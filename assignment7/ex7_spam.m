%% 
%  Assignment 7: Spam Classification with SVM
%
%  Instructions
%  ------------
% 
...
%  To use an SVM to classify emails into Spam vs Non-Spam, you first need
%  to convert each email into a vector of features. In this part, you will
%  implement the preprocessing steps for each email. You should complete
%  the code in processEmail.m to produce a word indices vector for a given
%  email.

fprintf('\nPreprocessing sample email (emailSample1.txt)\n');

% Extract Features
file_content = readFile('emailSample1.txt');
word_indices  = processEmail(file_content);

% Print Stats
fprintf('Word Indices: \n');
for i = 1:length(word_indices)
    fprintf(' %d', word_indices(i));
    if mod(i, 10) == 0
        fprintf('\n')
    end
end
...
%  that these words are the most likely indicators of spam.
%
...
%  own emails! In the source code, we have included spamSample1.txt,
%  spamSample2.txt, emailSample1.txt and emailSample2.txt as examples. 
%  The following code reads in one of these emails and then use your 
%  learned SVM classifier to determine whether the email is Spam or 
%  Not Spam

% Set the file to be read in (change this to spamSample2.txt,
% emailSample1.txt or emailSample2.txt to see different predictions 
% on different email types). Try your own emails as well!
